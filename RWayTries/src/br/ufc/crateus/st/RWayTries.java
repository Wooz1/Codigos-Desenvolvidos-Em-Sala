...
public class RWayTries<V> {
	...
	public RWayTries<Integer> countSubString(String l) {
		return countSubString(root, l);
	}

	private RWayTries<Integer> countSubString(Node r, String l) {
		RWayTries<Integer> tmp = new RWayTries<Integer>();
		if (r == null)
			return null;
		else {
			int a = 0;
			for (int i = 0; i < l.length(); i++) {
				if (i + 3 < l.length()) {
					String aux1 = l.substring(i, i + 1);
					String aux2 = l.substring(i + 1, i + 2);
					String aux3 = l.substring(i + 2, i + 3);
